...
`BUSINESS_QUERY` / `TASK_EXECUTION` 两类意图走多轮工具调用循环，工具统一注册在
`app/tools/registry.py`：

| 工具 | 用途 |
| --- | --- |
| `search_knowledge` | 检索制度文档，带版本与废止标记 |
| `query_order` | 按订单号 / 产品名 / 状态 / 客户名查询订单 |
| `query_customer` | 按姓名查询客户信息 |

设计要点：

- **多轮循环（上限 5 轮）**：支持「先查客户、再查该客户订单」这类多跳查询；
  超限后要求模型基于已有结果作答，不会无限调用。
- **`search_knowledge` 也是工具**，与业务工具共用注册/执行/审计链路，
  而不是在路由里硬编码一次检索调用。
- **工具异常转文本回传**：未知工具、参数错误、数据库异常都被捕获成 `tool` 消息，
  由模型决定怎么向用户解释，请求不会 500。
- **空回复兜底**：模型只返回 tool_calls、正文为空时，用户不会收到空字符串。

实测：

```
帮我查一下订单 A1001        → query_order，返回 企业版License / 19999.00 / paid
张三一共买了哪些产品？有退款吗？ → query_order(customer_name=张三)，识别 A1003 已退款
旗舰型设备整机保修多久？      → 走知识分支，36 个月 + 引用（不触发工具）
```
